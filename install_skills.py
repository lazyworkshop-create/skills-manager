...
def main():
    ...
    if args.global_install:
        ...
        if args.check_updates:
             # If check updates is called but no path, we ask where to check
             pass 
        else:
             target_dir = get_target_directory()

    # If still no target dir (e.g. check-updates interactive), ask again or infer
    if target_dir is None:
        target_dir = get_target_directory()

    print(f"\nManage Location: {target_dir}")
